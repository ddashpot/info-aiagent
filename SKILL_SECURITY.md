...
3. **汎用性**を保つ。
   ...
   - **外部ツールを組み合わせる**：garak / PyRIT ほかを `tool_registry.json` で脆弱性へマッピングし、
     `aiva coverage` で網羅性とギャップを算定。単独で全部やらず、確立ツールをMECE背骨へ正規化して束ねる。
     新ツールはレジストリに1行追加（実行・正規化したいときのみ `integrations.py` にアダプタ実装）。
